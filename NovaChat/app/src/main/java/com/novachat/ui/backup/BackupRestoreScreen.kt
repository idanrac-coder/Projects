...
private suspend fun exportDatabaseBackup(context: Context, targetUri: Uri): Boolean = withContext(Dispatchers.IO) {
    runCatching {
...
private suspend fun importDatabaseBackup(context: Context, sourceUri: Uri): Boolean = withContext(Dispatchers.IO) {
    runCatching {
