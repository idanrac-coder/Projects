...
object DeterministicSpamLayer {
    ...
    private val chainMessageRegex = Regex("העבר\\s*ל-?\\d+\\s*אנשימ|שלח\\s*לכל\\s*אנשי\\s*הקשר|העבר\\s*הלאה.*דחופ")
    private val serviceImpersonationRegex = Regex("(?i)(Wolt|10bis|Gett|Yango|Waze).*לחצ\\s*כאנ|(?i)(Wolt|10bis|תנ\\s*ביס).*הזמנ")
